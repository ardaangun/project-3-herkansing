...
if(isset($_POST['Wishlist'])){
    $lijstje = $_POST['removeId'];
    $_SESSION['lijst'][] = $lijstje;
}
?>
<title>Your Cart at GameINK</title>
<section id="cart">
        <nav id="cart"></nav>
        <h1 id="cartTitle">Mijn Cart</h1>
        <div class="cartBody">
        <div class="cart">
            <?php
            $total = 0;
                ...
                while ($row = mysqli_fetch_assoc($result)){
                    foreach ($itemId as $id){
                        if($row['id'] == $id){
                            ...
                            GameCart($row['naam'],$row['prijs'],$row['afbeelding'],$row['id'],$row['merken']);
                        }
                    }
                }
            ...
            <div class="cartSubtotal">
                <h1 id="cartSubtotal">Cart Geweren</h1>
                <h2 id="cartSubtotal"><?php if(isset($_SESSION['cart'])){
                    $count = count($_SESSION['cart']);
                    echo "Total Geweren: ".($count).""; }else{ echo "Price 0";}?>
                </h2>
                <?php
                $total = 0;
                if (isset($_SESSION['cart'])){
